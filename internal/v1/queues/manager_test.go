...
func TestQueueManager_Create(t *testing.T) {
	...
	t.Run("it returns an error if creating the queue errors", func(t *testing.T) {
		...
		// queue manager
		queueManager := queues.NewManager(fakeQueueConstructor)
		g.Expect(queueManager).ToNot(BeNil())

		err := queueManager.Create(logger, &v1.Create{Name: "test", QueueMaxSize: 5, ItemRetryAttempts: 0, DeadLetterQueueMaxSize: 0})
		g.Expect(err).To(HaveOccurred())
		g.Expect(err.Message).To(Equal("Failed to create queue"))
	...
	t.Run("it only creates a queue's tag group if it does not exist", func(t *testing.T) {
		...
		err := queueManager.Create(zap.NewNop(), &v1.Create{Name: "test", QueueMaxSize: 5, ItemRetryAttempts: 0, DeadLetterQueueMaxSize: 0})
...
func TestQueueManager_Find(t *testing.T) {
	...
	t.Run("it returns the proper queue", func(t *testing.T) {
		...
		g.Expect(queueManager.Create(zap.NewNop(), &v1.Create{Name: "test1", QueueMaxSize: 5, ItemRetryAttempts: 0, DeadLetterQueueMaxSize: 0})).ToNot(HaveOccurred())
		g.Expect(queueManager.Create(zap.NewNop(), &v1.Create{Name: "test2", QueueMaxSize: 5, ItemRetryAttempts: 0, DeadLetterQueueMaxSize: 0})).ToNot(HaveOccurred())
		g.Expect(queueManager.Create(zap.NewNop(), &v1.Create{Name: "test3", QueueMaxSize: 5, ItemRetryAttempts: 0, DeadLetterQueueMaxSize: 0})).ToNot(HaveOccurred())
		g.Expect(queueManager.Create(zap.NewNop(), &v1.Create{Name: "test4", QueueMaxSize: 5, ItemRetryAttempts: 0, DeadLetterQueueMaxSize: 0})).ToNot(HaveOccurred())
		g.Expect(queueManager.Create(zap.NewNop(), &v1.Create{Name: "test5", QueueMaxSize: 5, ItemRetryAttempts: 0, DeadLetterQueueMaxSize: 0})).ToNot(HaveOccurred())
...
func TestQueueManager_Metrics(t *testing.T) {
	...
	t.Run("it returns metrics for all queues", func(t *testing.T) {
		...
		// create 3 queues
		g.Expect(queueManager.Create(logger, &v1.Create{Name: "test1", QueueMaxSize: 5, ItemRetryAttempts: 0, DeadLetterQueueMaxSize: 0})).ToNot(HaveOccurred())
		g.Expect(queueManager.Create(logger, &v1.Create{Name: "test2", QueueMaxSize: 5, ItemRetryAttempts: 0, DeadLetterQueueMaxSize: 0})).ToNot(HaveOccurred())
		g.Expect(queueManager.Create(logger, &v1.Create{Name: "test3", QueueMaxSize: 5, ItemRetryAttempts: 0, DeadLetterQueueMaxSize: 0})).ToNot(HaveOccurred())
