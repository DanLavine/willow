...
// CanDelete is causing issues!
// when trying t figure out if an item can be deleted, removing that from the btree first cause an issue where 2 create
// requests come through, and 1 delete come through at the same time. In this case, the 2nd create can attempt to get the
// item from the ID tree which would fail. If thats the case, then do we need to create it? After that, Delete could remove
// the keys so that the ID is "lost"... this sucks
