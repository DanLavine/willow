...
import (
	...
	"github.com/DanLavine/willow/internal/server/serverfakes"
...
func TestLimiterTCP_ShutdownBehavior(t *testing.T) {
	...
	t.Run("It shuts down if there are no processing clients", func(t *testing.T) {
		...
		fakeLimitHandler := serverfakes.NewMockLimiterHandler(mockController)
	...
	t.Run("It waits for any in flight http requests", func(t *testing.T) {
		...
		defer mockController.Finish()
		fakeLimitHandler := serverfakes.NewMockLimiterHandler(mockController)
		fakeLimitHandler.EXPECT().CreateGroupRule(gomock.Any(), gomock.Any()).Do(func(w http.ResponseWriter, r *http.Request) {
			handlerChan <- struct{}{}
			handlerChan <- struct{}{}
