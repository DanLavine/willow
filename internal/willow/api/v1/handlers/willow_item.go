...
func (qh *queueHandler) Enqueue(w http.ResponseWriter, r *http.Request) {
	...
	case "POST":
		...
		if err := api.DecodeAndValidateHttpRequest(r, enqueueItem); err != nil {
...
func (qh *queueHandler) Dequeue(w http.ResponseWriter, r *http.Request) {
	...
	case "GET":
		...
		if err := api.DecodeAndValidateHttpRequest(r, dequeueItemRequest); err != nil {
			_, _ = api.EncodeAndSendHttpResponse(r.Header, w, err.StatusCode, err)
			return
		}
		...
		_, writeErr := api.EncodeAndSendHttpResponse(r.Header, w, http.StatusOK, dequeueItem)
		if writeErr == nil {
